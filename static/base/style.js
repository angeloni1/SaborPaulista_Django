    ...
    //         }
        ...
        */
        ...
        function addElementInCard(id, nome) {
            ...
            if (existeItem != -1) {
                itensCarrinho.splice(existeItem, 1);
            }
        ...
        function renderizarCarrinho(dados) {
            ...
            for (var i = 0; i <= dados.length -1; i++) {
                somaQtd += parseInt(dados[i].quantidade);
                novoProdutoCarrinho += '   <li id="produto' + i + '">' +
                    '<div class="tpcart__item">' +
                    ' <div class="tpcart__img">' +
                    ...
                    '  <div class="tpcart__content">' +
                    '<span class="tpcart__content-title">' +
                    '<a href="/produtos/' + dados[i]["id"] + '">' + dados[i]['nome'] + '</a>' +
                    '   </span>' +
                    '<a onclick="removeElementCart(' + i + ')" style="font-weight:bold; color:red; font-size:15px; margin-left:25px;">X</a>' +
                    '  <div class="tpcart__cart-price">' +
                    ' <span class="quantity">' + dados[i]['quantidade'] + ' x </span>' +
                    //' <span class="new-price">R$' + preco.textContent + '</span>' +
                    '   </div>' +
                    ' </div>' +
                    '  </div>' +
            ...
            localStorage.setItem("numberItens", Number(somaQtd));

            numeroItens.innerHTML = '<p style="color:white;">' + somaQtd + '</p>';
            numeroItensMobile.innerHTML = '<p style="color:white;">' + somaQtd + '</p>';
            
            var listaProdutos = document.getElementById("listaProducts");
            listaProdutos.innerHTML = novoProdutoCarrinho;
        }
